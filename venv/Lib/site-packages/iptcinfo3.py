...
def hex_dump(dump):
    ...
    """
    length = len(dump)
    def P(z): return chr(z) if ord3(z) >= 0x21 and ord3(z) <= 0x7e else '.'  # noqa: E731
    ROWLEN = 18
    res = ['\n']
...
def jpeg_next_marker(fh):
    ...
    logger.debug("jpeg_next_marker: at marker %02X (%d)",
                 ord3(byte), ord3(byte))
    return byte
...
def jpeg_skip_variable(fh, rSave=None):
    ...
    if rSave is not None or debugMode > 0:
        try:
            ...
            logger.error(
                "jpeg_skip_variable: read failed while skipping var data")
            return None
    else:
        # Just seek
        try:
...
def jpeg_collect_file_parts(fh, discard_app_parts=False):
    ...
    if marker == APP0 or not discard_app_parts:
        ...
        start.append(pack('8B', 0, 0, 0, 0, 0, 0, 0, 0))
    ...
    while True:
        ...
        # Check for start of compressed data
            ...
            logger.debug(
                "jpeg_collect_file_parts: saw start of compressed data")
            end.append(pack("BB", 0xff, ord3(marker)))
            break
...
class IPTCData(dict):
    ...
    def _key_as_str(cls, key):
        if isinstance(key, str) and key in c_datasets_r:
            ...
            raise KeyError("Key %s is not in %s!" %
                           (key, list(c_datasets.keys())))
...
class IPTCInfo:
        ...
        while offset <= MAX:
            ...
            if ord3(temp) == 0x1c:
                ...
                if record == 1 and dataset == 90:
                    ...
                    logger.debug(
                        "BlindScan: found IIM start at offset %d", offset)
        ...
        while True:
            ...
            if self.inp_charset:
                try:
                    ...
                    logger.warn('Data "%r" is not in encoding %s!',
    ...
    def _enc(self, text):
        ...
        if isinstance(text, str):
            ...
            try:
                ...
                logger.error("_enc: charset %s is not working for %s",
    ...
    def packedIIMData(self):
        ...
        for dataset, value in self._data.items():
            ...
            logger.debug('packedIIMData %02X: %r -> %r',
                         dataset, value, self._enc(value))
